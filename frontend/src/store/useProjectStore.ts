...
export const useProjectStore = create<ProjectState>((set, _get) => ({
  ...
  fetchProjects: async (communityId: string, pageNumber = 1, pageSize = 10) => {
    ...
    try {
      ...
      const { data } = await axios.get<PaginatedResponse<Project>>(`/community/${communityId}/project`, {
  ...
  fetchProjectById: async (communityId: string, projectId: string) => {
    ...
    try {
      ...
      const token = localStorage.getItem('token');
      const { data } = await axios.get<Project>(`/community/${communityId}/project/${projectId}`, {
        headers: token ? { Authorization: `Bearer ${token}` } : undefined,
      });
  ...
  createProject: async (communityId: string, data: CreateProjectData) => {
    ...
    try {
      ...
      const token = localStorage.getItem('token');
      const response = await axios.post<Project>(`/community/${communityId}/project`, data, {
        headers: token ? { Authorization: `Bearer ${token}` } : undefined,
      });
  ...
  updateProject: async (communityId: string, projectId: string, data: UpdateProjectData) => {
    ...
    try {
      ...
      const token = localStorage.getItem('token');
      const response = await axios.put<Project>(`/community/${communityId}/project/${projectId}`, data, {
        headers: token ? { Authorization: `Bearer ${token}` } : undefined,
      });
  ...
  deleteProject: async (communityId: string, projectId: string) => {
    ...
    try {
      ...
      const token = localStorage.getItem('token');
      await axios.delete(`/community/${communityId}/project/${projectId}`, {
        headers: token ? { Authorization: `Bearer ${token}` } : undefined,
      });
      set(state => ({
  ...
  joinProject: async (communityId: string, projectId: string) => {
    ...
    try {
      ...
      const token = localStorage.getItem('token');
      const response = await axios.post<ProjectMember>(`/community/${communityId}/project/${projectId}/join`, {}, {
        headers: token ? { Authorization: `Bearer ${token}` } : undefined,
      });
  ...
  leaveProject: async (communityId: string, projectId: string) => {
    ...
    try {
      ...
      await axios.post(`/community/${communityId}/project/${projectId}/leave`, {}, {
        headers: token ? { Authorization: `Bearer ${token}` } : undefined,
      });
      set(state => ({
        currentProject: state.currentProject?.id === projectId
  ...
  fetchProjectMembers: async (communityId: string, projectId: string, pageNumber = 1, pageSize = 10) => {
    ...
    try {
      ...
      const token = localStorage.getItem('token');
      const { data } = await axios.get<PaginatedResponse<User>>(`/community/${communityId}/project/${projectId}/members`, {
        params: { pageNumber, pageSize },
        headers: token ? { Authorization: `Bearer ${token}` } : undefined,
      });
  ...
  fetchProjectAnalytics: async (communityId: string, projectId: string) => {
    ...
    try {
      ...
      const token = localStorage.getItem('token');
      const { data } = await axios.get<ProjectAnalytics>(`/community/${communityId}/project/${projectId}/analytics`, {
        headers: token ? { Authorization: `Bearer ${token}` } : undefined,
      });
